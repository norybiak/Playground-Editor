...
{ 'use strict';
	...
	main.start = function(theScene, cb)
	{
		...
		Promise.all(promises).then(function(values) 
		{
			...
			//if (ppm <= 40)
			//{
			//	spawnOutOfEnclosure = true;	
			//}
					
			var skeletonExists = false;
	...
	main.update = function(timestamp)
	{
		if ((user.isModerator || globalEdit))
		{
			...
			if (isUserCurrentlyEditing)
			{
				...
				/*
				if ((pos.x + selectedObject.boundingBox.min.x) < (-enclosure.innerWidth / 2) + 5 || (pos.x - selectedObject.boundingBox.max.x) < (-enclosure.innerWidth / 2) + 5)
				{
					pos.x = ((-enclosure.innerWidth / 2) + 5) - selectedObject.boundingBox.min.x;
				}
				else if ((pos.x - selectedObject.boundingBox.min.x) > enclosure.innerWidth / 2 - 5)
				{
					pos.x = ((enclosure.innerWidth / 2) - 5) - selectedObject.boundingBox.max.x;
				}
				*/
				...
				else if ((pos.z - selectedObject.boundingBox.min.z)> enclosure.innerDepth / 2 - 5)
				{
					pos.z = ((enclosure.innerDepth / 2) - 5) - selectedObject.boundingBox.max.z;
				}
				*/
	...
	*///////////////////////
	...
	/**
	 * Load the object with transform information and call setup
	 * This uses the UltimateLoader library.
	 *
	 */
